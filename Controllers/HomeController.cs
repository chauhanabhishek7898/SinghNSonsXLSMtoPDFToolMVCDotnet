    ...
    public class HomeController : Controller
    {
            ...
            List<int> sheetOrders, List<string> sheetOrientations)
        {
            try
            {
                ...
                Console.WriteLine($"Sheet Orders: {string.Join(", ", sheetOrders ?? new List<int>())}");
                Console.WriteLine($"Sheet Orientations: {string.Join(", ", sheetOrientations ?? new List<string>())}");
                ...
                {
                    Console.WriteLine("ℹ️ No orientation mapping provided");
                }

                var outputFileName = System.IO.Path.GetFileNameWithoutExtension(originalFileName ?? "converted") + ".pdf";

                // Step 1: Convert Excel to PDF with selected sheets, order and orientation
                Console.WriteLine($"🔄 Starting Excel to PDF conversion...");
                var result = await _libreOfficeService.ConvertToPdfAsync(filePath, outputFileName, orderedSheets, orientationMap);

                if (result.Success)
                {
                    Console.WriteLine($"✅ Excel to PDF conversion successful: {result.PdfFilePath}");

                    // Step 2: Merge with existing PDFs using iText7
...
//                var outputFileName = Path.GetFileNameWithoutExtension(originalFileName) + ".pdf";

//                // Convert to PDF with order and orientation
//                var result = await _libreOfficeService.ConvertToPdfAsync(filePath, outputFileName, orderedSheets, orientationMap);

//                if (result.Success)
//                {
//                    // Merge with existing PDFs from D:\CIPL\SinghAndSons\pdf
//                    var mergedPdfResult = await MergeWithExistingPdfs(result.PdfFilePath, outputFileName);

//                    // Cleanup session
//                    HttpContext.Session.Remove(sessionId + "_filePath");
//                    HttpContext.Session.Remove(sessionId + "_fileName");

//                    if (mergedPdfResult.Success)
//                    {
//                        // Return merged PDF file
//                        var fileBytes = await System.IO.File.ReadAllBytesAsync(mergedPdfResult.PdfFilePath);

//                        // Cleanup both original and merged PDF files
//                        System.IO.File.Delete(result.PdfFilePath);
//                        System.IO.File.Delete(mergedPdfResult.PdfFilePath);

//                        return File(fileBytes, "application/pdf", mergedPdfResult.FileName);
//                    }
//                    else
//                    {
//                        // Return original PDF if merge failed
//                        var fileBytes = await System.IO.File.ReadAllBytesAsync(result.PdfFilePath);
//                        System.IO.File.Delete(result.PdfFilePath);
//                        return File(fileBytes, "application/pdf", result.FileName);
//                    }
...
//        // NEW: Method to merge with existing PDFs using simple file copy approach
//        private async Task<ConversionResult> MergeWithExistingPdfs(string newPdfPath, string outputFileName)
//        {
//            var result = new ConversionResult();
...
//                {
//                    result.Success = true; // No existing PDFs to merge
//                    result.PdfFilePath = newPdfPath;
//                    result.FileName = outputFileName;
//                    return result;
//                }
...
//                Console.WriteLine($"Found {existingPdfFiles.Length} existing PDF files to merge");

//                // Simple approach: Copy all PDFs to a zip file instead of merging
//                var mergedZipPath = Path.Combine(Path.GetTempPath(), $"merged_{Guid.NewGuid()}.zip");
//                var mergedFileName = $"merged_{Path.GetFileNameWithoutExtension(outputFileName)}_{DateTime.Now:yyyyMMdd_HHmmss}.zip";

//                using (var zipStream = new System.IO.Compression.ZipArchive(System.IO.File.Create(mergedZipPath), System.IO.Compression.ZipArchiveMode.Create))
//                {
//                    // Add existing PDFs
//                    foreach (var existingPdf in existingPdfFiles)
//                    {
//                        try
//                        {
//                            var entry = zipStream.CreateEntry(Path.GetFileName(existingPdf));
//                            using (var entryStream = entry.Open())
//                            using (var fileStream = System.IO.File.OpenRead(existingPdf))
//                            {
//                                await fileStream.CopyToAsync(entryStream);
//                            }
//                            Console.WriteLine($"✅ Added existing PDF to zip: {Path.GetFileName(existingPdf)}");
//                        }
//                        catch (Exception ex)
//                        {
//                            Console.WriteLine($"❌ Error adding existing PDF {existingPdf}: {ex.Message}");
...
//                    // Add the new converted PDF
//                    try
//                    {
//                        var entry = zipStream.CreateEntry(Path.GetFileName(newPdfPath));
//                        using (var entryStream = entry.Open())
//                        using (var fileStream = System.IO.File.OpenRead(newPdfPath))
//                        {
//                            await fileStream.CopyToAsync(entryStream);
//                        }
//                        Console.WriteLine($"✅ Added new converted PDF to zip: {Path.GetFileName(newPdfPath)}");
//                    }
//                    catch (Exception ex)
//                    {
...
//                result.Success = true;
//                result.PdfFilePath = mergedZipPath;
//                result.FileName = mergedFileName;
//                result.Message = $"Successfully packaged {existingPdfFiles.Length + 1} PDF files into zip";

//                Console.WriteLine($"✅ PDF packaging completed: {result.Message}");

//                return result;
//            }
//            catch (Exception ex)
//            {
//                Console.WriteLine($"❌ Error packaging PDFs: {ex}");
//                result.Success = false;
//                result.Message = $"Packaging failed: {ex.Message}";
//                result.PdfFilePath = newPdfPath; // Return original PDF
//                result.FileName = outputFileName;
//                return result;
//            }
//        }
