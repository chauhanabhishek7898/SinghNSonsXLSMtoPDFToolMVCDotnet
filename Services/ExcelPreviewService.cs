...
namespace ExcelToPdfConverter.Services
{
    public class ExcelPreviewService
    {
        ...
        // नया: Find Invoice Numbers in worksheet
        ...
        //            // Check for invoice number text
        //            if (IsInvoiceNumberText(cellValue))
        //            {
        //                Console.WriteLine($"📍 Found 'INVOICE NUMBER' text at {GetColumnName(col)}{row}: '{cellValue}'");

        //                // Look for invoice number in multiple possible locations
        //                var invoiceNumberValue = FindInvoiceNumberValue(worksheet, row, col, maxRows, maxCols);
        ...
        //                else
        //                {
        //                    Console.WriteLine($"❌ No invoice number found near 'INVOICE NUMBER' at {GetColumnName(col)}{row}");
        //                }
        //            }

        //            // Also check if cell itself looks like an invoice number
        //            else if (IsInvoiceNumberValue(cellValue))
        //            {
        //                invoiceNumbers.Add(new InvoiceNumber
        //                {
        //                    SheetName = worksheet.Name,
        ...
        //                });
        //                Console.WriteLine($"✅ Auto-detected invoice number at {GetColumnName(col)}{row}: '{cellValue.Trim()}'");
        //            }
        //        }
        //    }
        ...
        //    // Contains check (with word boundaries)
        ...
        //private string FindInvoiceNumberValue(ExcelWorksheet worksheet, int row, int col, int maxRows, int maxCols)
        //{
        ...
        //                if (!string.IsNullOrEmpty(checkValue) && IsInvoiceNumberValue(checkValue))
        //                {
        //                    Console.WriteLine($"   ✅ Found invoice number after merged area at {GetColumnName(checkCol)}: '{checkValue}'");
        //                    return checkValue;
        //                }
        //            }
        //        }
        ...
        //        if (!string.IsNullOrEmpty(rightValue) && IsInvoiceNumberValue(rightValue))
        //        {
        //            Console.WriteLine($"   ✅ Found invoice number in immediate right cell: '{rightValue}'");
        //            return rightValue;
        //        }
        //    }
        ...
        //        if (!string.IsNullOrEmpty(skipValue) && IsInvoiceNumberValue(skipValue))
        //        {
        //            Console.WriteLine($"   ✅ Found invoice number by skipping one column: '{skipValue}'");
        //            return skipValue;
        //        }
        //    }

        //    // CASE 4: Try cell below
        //    int belowRow = row + 1;
        //    if (belowRow <= maxRows)
        //    {
        ...
        //        if (!string.IsNullOrEmpty(belowValue) && IsInvoiceNumberValue(belowValue))
        //        {
        //            Console.WriteLine($"   ✅ Found invoice number in cell below: '{belowValue}'");
        //            return belowValue;
        //        }
        //    }
        ...
        //private bool IsInvoiceNumberValue(string value)
        //{
        ...
        //    // 1. Starts with INV or IN
        //    //if (trimmedValue.StartsWith("INV", StringComparison.OrdinalIgnoreCase) ||
        //    //    trimmedValue.StartsWith("IN-", StringComparison.OrdinalIgnoreCase) ||
        //    //    trimmedValue.StartsWith("IN/", StringComparison.OrdinalIgnoreCase))
        //    //{
        //    //    return true;
        //    //}

        //    // 2. Contains digits (invoice numbers often have numbers)
        //    if (Regex.IsMatch(trimmedValue, @"\d"))
        //    {
        //        // Check length - invoice numbers are usually not too long
        //        if (trimmedValue.Length >= 3 && trimmedValue.Length <= 50)
        //        {
        //            // Don't match if it's clearly a date
        //            if (!IsPotentialDate(trimmedValue) &&
        //                !trimmedValue.Contains(",") && // Not a date with comma
        //                !trimmedValue.ToLower().Contains("date"))
        //            {
        //                return true;
        //            }
        //        }
        ...
        //// नया: Find Date Ranges in worksheet (01-10-2025 TO 31-10-2025 format)
        ...
        //            // Check for date range pattern: 01-10-2025 TO 31-10-2025
        //            if (IsDateRangePattern(cellValue))
        //            {
        ...
        //private bool IsDateRangePattern(string value)
        //{
        //    if (string.IsNullOrEmpty(value)) return false;
        ...
        //    var pattern1 = @"^\d{1,2}[-/]\d{1,2}[-/]\d{4}\s+TO\s+\d{1,2}[-/]\d{1,2}[-/]\d{4}$";

        //    // Pattern: mm-dd-yyyy TO mm-dd-yyyy
        //    var pattern2 = @"^\d{1,2}[-/]\d{1,2}[-/]\d{4}\s+TO\s+\d{1,2}[-/]\d{1,2}[-/]\d{4}$";

        //    // Pattern with month names: 01 October 2025 TO 31 October 2025
        //    var pattern3 = @"^\d{1,2}\s+(JANUARY|FEBRUARY|MARCH|APRIL|MAY|JUNE|JULY|AUGUST|SEPTEMBER|OCTOBER|NOVEMBER|DECEMBER)\s+\d{4}\s+TO\s+\d{1,2}\s+(JANUARY|FEBRUARY|MARCH|APRIL|MAY|JUNE|JULY|AUGUST|SEPTEMBER|OCTOBER|NOVEMBER|DECEMBER)\s+\d{4}$";

        //    // Pattern: 01/10/2025 TO 31/10/2025 (with slashes)
        //    var pattern4 = @"^\d{1,2}/\d{1,2}/\d{4}\s+TO\s+\d{1,2}/\d{1,2}/\d{4}$";

        //    return Regex.IsMatch(trimmedValue, pattern1, RegexOptions.IgnoreCase) ||
        ...
        // नया: Find Invoice Numbers in worksheet - ONLY exact "INVOICE NUMBER" text
        //    private List<InvoiceNumber> FindInvoiceNumbersInWorksheet(ExcelWorksheet worksheet)
        //    {
        //        var invoiceNumbers = new List<InvoiceNumber>();
        ...
        //                // Check for EXACT "INVOICE NUMBER" text only (case-insensitive)
        //                if (IsExactInvoiceNumberText(cellValue))
        //                {
        //                    Console.WriteLine($"📍 Found 'INVOICE NUMBER' text at {GetColumnName(col)}{row}: '{cellValue}'");

        //                    // Look for invoice number in the cell to the RIGHT
        //                    var invoiceNumberValue = FindInvoiceNumberToRight(worksheet, row, col, maxCols);
        ...
        //                    else
        //                    {
        //                        Console.WriteLine($"❌ No invoice number found to the right of 'INVOICE NUMBER' at {GetColumnName(col)}{row}");
        //                    }
        //                }
        //            }
        //        }
        ...
        //    private bool IsExactInvoiceNumberText(string cellValue)
        //    {
        //        if (string.IsNullOrEmpty(cellValue)) return false;
        ...
        //    "Invoice Number",
        //    "invoice number",
        //    "INVOICE NUMBER\t", // With tab
        //    "Invoice Number\t",
        //    "invoice number\t"
        //};

        //        // Remove any trailing tabs or whitespace for comparison
        //        var cleanValue = trimmedValue.TrimEnd('\t', ' ', '\r', '\n');

        //        // Check exact match
        //        return exactPatterns.Any(pattern =>
        //            string.Equals(cleanValue, pattern.TrimEnd('\t', ' ', '\r', '\n'), StringComparison.OrdinalIgnoreCase));
        //    }

        //    private string FindInvoiceNumberToRight(ExcelWorksheet worksheet, int row, int col, int maxCols)
        //    {
        ...
        //            {
        //                Console.WriteLine($"   ✅ Found value in right cell {GetColumnName(rightCol)}: '{rightValue}'");
        //                return rightValue;
        //            }
        //        }
        ...
        //        int rightCol2 = col + 2;
        //        if (rightCol2 <= maxCols)
        //        {
        //            var rightCell2 = worksheet.Cells[row, rightCol2];
        //            var rightValue2 = GetCellValue(rightCell2);

        //            if (!string.IsNullOrEmpty(rightValue2))
        //            {
        //                Console.WriteLine($"   ✅ Found value in second right cell {GetColumnName(rightCol2)}: '{rightValue2}'");
        //                return rightValue2;
        //            }
        //        }
        ...
        private List<InvoiceNumber> FindInvoiceNumbersInWorksheet(ExcelWorksheet worksheet)
        {
            ...
            for (int row = 1; row <= maxRows; row++)
            {
                for (int col = 1; col <= maxCols; col++)
                {
                    ...
                    // Check if cell contains "INVOICE NUMBER" text (case-insensitive)
                    if (ContainsInvoiceNumberText(cellValue))
                    {
                        Console.WriteLine($"📍 Found cell containing 'INVOICE NUMBER' at {GetColumnName(col)}{row}: '{cellValue}'");
                        ...
                        if (!string.IsNullOrEmpty(invoiceNumberValue))
                        {
                            ...
                            Console.WriteLine($"✅ CONFIRMED invoice number: 'INVOICE NUMBER' → '{invoiceNumberValue.Trim()}'");
                        }
                        else
                        {
                            Console.WriteLine($"❌ No invoice number found to the right of 'INVOICE NUMBER' at {GetColumnName(col)}{row}");
            ...
            {
                Console.WriteLine("📋 Found invoice numbers:");
                foreach (var inv in invoiceNumbers)
                {
                    Console.WriteLine($"   • {inv.Number} at {inv.SheetName}:{inv.Location}");
                }
            }
        ...
        private bool ContainsInvoiceNumberText(string cellValue)
        {
            if (string.IsNullOrEmpty(cellValue)) return false;

            var upperValue = cellValue.ToUpper();

            // Simple contains check for "INVOICE NUMBER"
            return upperValue.Contains("INVOICE NUMBER");
        }
        ...
        //            // Check for date range pattern: 01-10-2025 TO 31-10-2025
        //            if (IsExactDateRangePattern(cellValue))
        //            {
        //                Console.WriteLine($"📍 Found date range at {GetColumnName(col)}{row}: '{cellValue}'");
        ...
        //private bool IsExactDateRangePattern(string value)
        //{
        //    if (string.IsNullOrEmpty(value)) return false;

        //    var trimmedValue = value.Trim();

        //    // Exact pattern: dd-mm-yyyy TO dd-mm-yyyy with possible variations
        //    var pattern1 = @"^\d{1,2}-\d{1,2}-\d{4}\s+TO\s+\d{1,2}-\d{1,2}-\d{4}$";

        //    // Alternative with slashes: dd/mm/yyyy TO dd/mm/yyyy
        //    var pattern2 = @"^\d{1,2}/\d{1,2}/\d{4}\s+TO\s+\d{1,2}/\d{1,2}/\d{4}$";

        //    // Pattern with spaces: 01-10-2025 TO 31-10-2025
        //    var pattern3 = @"^\d{2}-\d{2}-\d{4}\s+TO\s+\d{2}-\d{2}-\d{4}$";

        //    return Regex.IsMatch(trimmedValue, pattern1) ||
        //           Regex.IsMatch(trimmedValue, pattern2) ||
        //           Regex.IsMatch(trimmedValue, pattern3);
        //}
